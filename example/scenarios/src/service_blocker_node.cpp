...
bool heavyService(std_srvs::Empty::Request &req,
  ...
  // Simulate a long-running service task (e.g., complex calculation, hardware
  // sync)
...
int main(int argc, char **argv) {
  ...
  ros::ServiceServer service =
      n.advertiseService("heavy_service", heavyService);
  ros::Subscriber sub = n.subscribe("blocked_topic", 1000, topicCallback);

  ROS_INFO("Ready to demonstrate Queuing Latency. Call /heavy_service to block "
           "the thread.");
